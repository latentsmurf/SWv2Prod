...
import { getBaseUrl } from "../../../../../components/reactvideoeditor/pro/utils/general/url-helper";
import { uploadFileToGCS } from "../../../../../lib/storage/gcs";
...
export async function startRendering(
  ...
  (async () => {
    try {
      ...
      // Upload to GCS
      console.log(`Uploading render ${renderId} to GCS...`);
      const fileBuffer = fs.readFileSync(path.join(VIDEOS_DIR, `${renderId}.mp4`));
      ...
      // Clean up local file
      fs.unlinkSync(path.join(VIDEOS_DIR, `${renderId}.mp4`));

      completeRender(renderId, gcsUrl, stats.size);
      console.log(`Render ${renderId} completed and uploaded to ${gcsUrl}`);
    } catch (error: any) {
